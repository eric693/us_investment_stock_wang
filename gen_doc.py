...
meta.add_run('版本日期：2026-08-15（新增大盤儀表板、自選報價、K 線副圖與逐日籌碼）').font.size = Pt(10)
...
# ===== 目錄式總覽 =====
h1('一、平台總覽')
para('本平台是一套以網頁操作的個人投資決策系統，共分為三大類、十四個功能模組。所有頁面共用上方導覽列，可隨時切換。每一檔股票（台股或美股）在各頁清單中皆可點擊，滑出「個股詳情面板」查看 K 線圖、技術指標、新聞與籌碼。')
doc.add_paragraph()
...
ov_rows = [
    ('市場儀表', '大盤儀表板', '加權指數走勢、大盤體質、法人與類股排行'),
    ('市場儀表', '自選報價', '一頁掃完所有關注標的的報價與距買點'),
...
bullet('十字游標（看盤 App 的標準操作）：滑鼠移到（手機則手指滑過）任一根 K 棒，會畫出十字虛線並在左側標出該根收盤價；圖上方同步顯示日期、開高低收、對前一根的漲跌幅、成交量，以及該根的 MA5 / MA20 / MA60 / 年線數值。移開游標自動回到最新一根。')
bullet('分時分頁：今日 5 分鐘走勢線＋當日累計均價（VWAP）線＋昨收虛線基準，下方為分時成交量；非交易時段自動顯示最近一個交易日。價格在均價之上代表當日買方掌控，跌破均價且量放大要留意賣壓。收盤集合競價會另補一根「收盤」點，讓最後價格與各頁的收盤價一致。')
bullet('技術指標副圖：K 線下方可切換 MACD（柱狀 + DIF / DEA）、KD 雙線、RSI，或關閉副圖；金叉、死叉、背離發生在哪一根一眼可見，十字游標會同步顯示該根的指標數值。')
bullet('我的買賣點標記：自己的買進點以紅色向上箭頭（B）、賣出點以綠色向下箭頭（S）標在 K 線上，持倉成本另以黃色虛線畫一條水平線，回頭檢討賣飛或追高最直觀。')
...
h2('籌碼分頁（台股）')
para('籌碼分頁分成「當日快照」與「逐日趨勢」兩層，看法人是持續買還是單日進出。')
bullet('當日快照：最近交易日的外資 / 投信 / 自營商 / 三大法人合計買賣超（張），加融資融券餘額與增減、借券賣出餘額、當沖比。')
bullet('逐日籌碼表：近 10 個交易日一天一列，欄位為外資 / 投信 / 自營 / 合計買賣超（張）＋融資增減 / 融券增減，紅色為買超、綠色為賣超。')
bullet('融資增減採「反向著色」：融資增加代表散戶追價、籌碼變鬆，視為偏空而顯示綠色；融資減少（散戶退場）顯示紅色。')
bullet('表尾兩列累計：近 5 日與近 10 日各法人合計，並標示「買超 N / M 日」，用來判斷法人買超是否連續。')
bullet('此頁純資料查詢、不呼叫 AI、不耗 Token；點開籌碼分頁時才載入，不會拖慢面板開啟速度。資料源為 FinMind，快取 1 小時（籌碼本來就是日更）。')
doc.add_paragraph()
...
# 2b 大盤儀表板
h2('2-1. 大盤儀表板')
kv_table([('路徑', '/market'), ('定位', '先看大盤、再看個股：判斷今天適不適合進場')])
para('重點功能', bold=True, color=ACCENT)
bullet('加權指數 K 線：日 K 蠟燭圖含 MA20 / MA60 / 年線與成交量，滑鼠移過去顯示該日開高低收與漲跌幅，可切換近 3 月 / 6 月 / 1 年。')
bullet('大盤體質濾網：綜合指數與月線 / 季線位置、月線方向與隔夜美股偏向，判定多頭 / 震盪 / 空頭，並直接告訴你現在該積極還是保守。')
bullet('指數卡片：加權、費半、那斯達克、標普、道瓊、台積電 ADR、VIX、美元台幣一次看完。')
bullet('三大法人對大盤買賣超（億元，近 12 個交易日）＋近 5 日外資合計，看外資是連買還是連賣。')
bullet('外資台指期未平倉淨口數（近 12 日）：正值為淨多單、負值為淨空單，重點看方向變化。')
bullet('類股族群漲跌排行：21 個族群的今日與近 5 日平均漲跌幅、上漲家數與今日領漲個股，看資金流向哪裡；點領漲個股可直接開詳情面板。')
para('操作步驟', bold=True, color=ACCENT)
bullet('開盤前或盤中先看這頁：體質標籤是「空頭」時就降低進場積極度。')
bullet('看自己持股所屬族群是否在領漲；整族群翻黑通常是輪動，不必然是個股變壞。')
bullet('全頁純資料查詢，不呼叫 AI、不耗 Token；類股排行首次計算需數十秒，之後 10 分鐘內走快取。')
doc.add_paragraph()

# 2c 自選報價
h2('2-2. 自選報價')
kv_table([('路徑', '/watch'), ('定位', '看盤 App 式的自選清單，一頁掃完所有關注標的')])
para('重點功能', bold=True, color=ACCENT)
bullet('一張表列出代號、名稱、現價、漲跌幅、持倉損益 %、成本、距買點 %、買點區間。')
bullet('分頁切換：全部 / 持倉 / 監測中 / 自訂分組；表頭可點擊排序（例如依漲跌幅或距買點排序）。')
bullet('標籤提示：持倉、監測、以及「在區間」（現價已落入 AI Agent 訂的買點範圍）。')
bullet('自訂分組可新增 / 編輯 / 刪除，存在伺服器設定檔，換裝置也看得到。')
bullet('每 60 秒自動更新一次，也可按「立即更新」；點任一列開啟詳情面板看 K 線與籌碼。')
bullet('除權息與財報行事曆：關注個股的除息日、現金 / 股票股利、現價殖利率與發放日，另附台股財報公告法定截止日提醒。')
para('操作步驟', bold=True, color=ACCENT)
bullet('先按「編輯自選分組」建立自己的分組（代號逗號分隔），儲存後上方就會多一個分頁。')
bullet('盤中用「距買點 %」排序，最接近買點的標的會排在前面。')
bullet('此頁純行情查詢，不呼叫 AI、不耗 Token。')
doc.add_paragraph()
