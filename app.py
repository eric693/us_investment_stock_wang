...
def _perpetual_metrics(ticker):
    """輕量版：算位階、距高點回檔、起漲動能分，供選股掃描用。快取 15 分鐘。"""
    ...
    if holding:
        ...
        if low_pos and above20 and entry_score >= 4 and pnl < 10:
            return out('BUY', '低位階加碼', '中',
                f'持倉仍在低位階（{pos:.0f}%）、未明顯起漲（{pnl:+.1f}%），且起漲動能轉強'
...
_PREDICT_SYSTEM_PROMPT = """你是一位精通台股技術分析、籌碼追蹤與總經連動的專業交易員助理。
你的核心優勢，是同時掌握「即時行情數據 + 隔夜美股 + 個股新聞」三類一般聊天機器人拿不到的資料，
因此你的明日預測必須超越泛泛而談——要把這些即時資訊「串成因果」，做出有市場感、可執行的研判，
而不是逐項複述指標。系統會自動取得以下數據供你分析：
...
【長期位階（永動機視角，5 年）】
- 5 年區間位階 %、距 52 週高點回檔 %、股價是否在年線(MA240)之下
- 這是「長線位階」，用來判斷這檔是不是還在相對歷史底部區（永動機選股的核心）

【初升段防禦檢查（5 項）】（短線「初升段、不過熱」的精準辨識；這不是選股頁的「起漲動能分」，時間維度與長期位階不同）
...
【法人籌碼】外資、投信、自營商買賣超、法人連續買賣超天數、借券趨勢
【基本面】月營收 YoY/MoM、最新季度 EPS
【市場與總經背景】隔夜美股（費城半導體/那指/標普/道瓊/台積電 ADR）、美元台幣、台股加權指數、對台股偏向
【近期新聞】Google News 個股近期標題（可能的催化劑或利空）

分析時請嚴格依照以下格式輸出：

### 今日消息面與盤勢連動（最重要，先講）
- **隔夜美股與系統性影響：** 用我提供的隔夜美股數據說明——尤其前一晚美股若大跌（例如費半重挫），
  研判它對「這檔股票所屬產業」的系統性衝擊有多大。半導體/電子權值股要重看費半與台積電 ADR；
  金融/傳產/內需則受美股直接影響較小，要點出這個差異，做相對比較（這檔比大盤抗跌還是更脆弱？）。
- **個股催化劑：** 若新聞標題有具體題材或利空，說明它與股價的關係；資料不足就誠實說「無明確個股消息」，
  絕不可捏造新聞或事件。
- **連動定調：** 綜合判斷明日這檔是「跟著大盤走」「逆勢有題材」還是「利空獨弱」。

### 綜合趨勢評估
- **長期位階定調：** 先用「長期位階（永動機視角）」說明這檔屬於相對低位階 / 中位階 / 相對高位階；若在年線之下或位階 < 40%，要明確點出「長線仍在永動機選股的相對低位階區」。
- **初升段防禦檢查符合度：** X/5 項（列出符合與不符合的項目）。注意：這組是「短線初升段、不過熱」的檢查，與選股頁的「起漲動能分」是兩回事；當長期位階仍低、但短線乖離/反彈已偏高時，正確說法是「長線低位階仍成立，惟短線已彈一段、乖離偏高，宜留意拉回或分批」，**不要因為防禦檢查不符合就反過來否定『這是低位階起漲股』**——長線位階與短線過熱是兩個維度，可以同時成立。
- **基本基調：** 偏多上攻 / 震盪洗盤 / 偏空回檔（需與上面的消息面定調一致）

### 明日實戰預測
- **預估漲跌幅區間：** 例如 +1.5% ~ +3.0%（須把隔夜美股偏向納入，例如美股大跌則開盤先讓一段）
- **多方攻擊關鍵壓力價：** [價位 + 技術依據]
- **空方防守關鍵支撐價：** [價位 + 技術依據]

### 各指標詳細分析
（逐項說明均線扣抵、量能、借券、KD、MACD、法人連續、基本面營收/EPS）

### 主要風險
（列出 2-3 點需要注意的風險，至少一點要連結到當前總經/產業系統性風險）

研判原則：
- 長期位階與短線乖離/反彈是不同時間維度，務必分開研判：一檔長線在年線之下、5 年位階偏低的股票，即使短線已彈一段、乖離 > 6%、反彈 > 15%，它「仍是低位階股」，只是短線進場點偏熱——此時該談的是「等拉回再分批」而非「漲一大段、不是起漲、整體偏空」。避免出現「永動機選它為低位階起漲，但 AI 卻說它過熱不該追」這種自相矛盾的結論。
...
def _fetch_predict_data(code: str) -> dict:
    ...
    try:
        ...
        # 長期位階（永動機視角）：與「低位階永動機」選股頁用同一套 5 年位階＋年線計算，
        # 讓 AI 預測能與選股邏輯對齊，不會把「長期低位階、短線剛彈」誤判成「漲一大段過熱」。
        try:
            perp = _perpetual_metrics(ticker_yf)
            if perp:
                result['perp_pos']        = perp.get('pos')        # 5 年區間位階 %
                result['perp_drawdown']   = perp.get('drawdown')   # 距 52 週高點回檔 %
                result['perp_below_year'] = perp.get('belowYear')  # 是否在年線(MA240)之下
                result['perp_rise_score'] = perp.get('riseScore')  # 永動機 5 項起漲分
        except Exception:
            pass
...
def _build_analysis_context(data: dict) -> str:
    ...
    # 長期位階（永動機視角）：給 AI 長線位階，避免只看短線乖離/反彈就誤判過熱
    pos = data.get('perp_pos')
    if pos is not None:
        below_year = data.get('perp_below_year')
        ddn = data.get('perp_drawdown')
        if pos < 40 or below_year:
            stage = '相對低位階（永動機選股區）'
        elif pos < 65:
            stage = '中位階'
        else:
            stage = '相對高位階'
        lines.append(f"\n### 【長期位階（永動機視角，5 年）】")
        lines.append(f"- 5 年區間位階：{pos:.0f}%（{stage}）")
        if ddn is not None:
            lines.append(f"- 距 52 週高點回檔：{ddn:+.0f}%")
        lines.append(f"- 年線(MA240)位置：{'股價在年線之下（相對低位階）' if below_year else '股價在年線之上'}")
        rscore = data.get('perp_rise_score')
        if rscore is not None:
            lines.append(f"- 起漲動能分：{rscore}/5（選股頁判定進場用的『右側轉強動能』，5 項＝站上月線、月線翻揚、帶量(量比≥1.3)、MACD動能轉強、KD黃金交叉）")
        lines.append(f"- 重要：『起漲動能分』(上面) 與『初升段防禦檢查』(下方) 是兩套定義不同的指標，請勿混為一談——"
                     f"(A) 起漲動能分＝右側『轉強動能』，選股頁就是用它（位階低＋此分≥3）判定『低位階起漲、建議進場』；"
                     f"(B) 初升段防禦檢查＝春燕式短線『不過熱』檢查（乖離<6%、反彈<15%、當沖<40% 等）。"
                     f"兩者時間維度不同、可同時成立：一檔被選股頁標為『低位階起漲』(A 達標)，其『初升段防禦檢查』(B) 未必滿足，這不是矛盾。"
                     f"嚴禁用 B 未達標去否定 A 的『這是低位階起漲股』結論，避免出現『選股頁選它、AI 卻說它不是起漲』的自相矛盾；"
                     f"B 未達標時正確說法是『長線低位階＋起漲動能仍成立，惟短線已偏熱，宜等拉回再分批』。")

    # 初升段防禦檢查（5 項，短線不過熱檢查；與選股頁「起漲動能分」是兩回事）
    lines.append(f"\n### 【初升段防禦檢查（5 項）】")
    ...
    if ld:
        ...
        chg  = bal - prev
        trend = '增加（空方加碼）' if chg > 0 else ('減少（空方回補，偏多）' if chg < 0 else '持平')
    ...
    if ih and ih.get('foreign'):
        def _streak(seq):
            ...
            return n * sign
        def _desc(n):
            return f'連{n}日買超' if n > 0 else (f'連{abs(n)}日賣超' if n < 0 else '無連續')
        fs = _streak(ih.get('foreign')); ts = _streak(ih.get('trust'))
        lines.append(f"- 法人連續：外資 {_desc(fs)} / 投信 {_desc(ts)}")

    # 基本面：月營收 YoY/MoM、EPS（讓 AI 判斷漲跌是否有基本面支撐）
    mr = data.get('month_rev')
    ep = data.get('eps')
    if mr or ep:
        lines.append(f"\n### 【基本面】")
        if mr:
            rev_yi = safe_float(mr.get('revenue', 0)) / 1e8
        ...
        if ep:
            ...
            lines.append(f"- 最新季度 EPS（{ep.get('quarter','')}）：{eps_v:.2f} 元")

    # 市場與總經背景 + 個股新聞（隔夜美股/費半/台股加權＋Google News 標題）
    # 這是相對 ChatGPT/Gemini 的關鍵優勢：餵入即時行情與新聞，讓 AI 做系統性連動研判。
...
def _fetch_kline(code: str, is_tw: bool = True, period: str = '6mo', interval: str = '1d') -> dict:
    """輕量 K 線資料：回傳近期 OHLCV＋MA5/20/60，供詳情面板畫圖。
    interval='1d' 日K（period 3mo/6mo），'1wk' 週K（取近 ~70 週、MA 為週均）。
    台股自動嘗試 .TW→.TWO；美股直接用代碼。快取 15 分鐘。不使用 anthropic、純行情。"""
    code = code.strip().upper().replace('.TW', '').replace('.TWO', '')
    # view = 顯示窗；不論顯示多久，日K 都抓 2 年歷史，確保年線 MA240（需 ≥240 根）算得正確。
    if interval == '1wk':
        interval = '1wk'; view = 'wk'; keep = 70; fetch_period = '2y'
    else:
        interval = '1d'
    ...
    if cached is not None:
        return cached
    out = {'code': code, 'candles': [], 'error': None, 'period': view, 'interval': interval}
    try:
        symbols = ([code + '.TW', code + '.TWO'] if is_tw else [code])
        hist = None
        for sym in symbols:
        ...
        # 年線：日K 用 MA240（需滿 240 根才是真正年線，不足則不畫）；週K 用 MA52（52 週≈一年）
        if interval == '1d':
        ...
        for t in idx:
            row = {
                't':  t.strftime('%Y-%m-%d'),
                'o':  round(safe_float(hist['Open'].loc[t]), 2),
                'h':  round(safe_float(hist['High'].loc[t]), 2),
                'l':  round(safe_float(hist['Low'].loc[t]), 2),
                'c':  round(safe_float(hist['Close'].loc[t]), 2),
                'v':  int(safe_float(hist['Volume'].loc[t])),
                'm5':  round(safe_float(ma5.loc[t]),  2),
                'm20': round(safe_float(ma20.loc[t]), 2),
        ...
        out['candles'] = candles
        out['is_tw'] = is_tw
        # 年線標籤與最新值（供前端圖例與「位階」說明）
        last_year = next((c['myear'] for c in reversed(candles) if c.get('myear') is not None), None)
        if last_year is not None:
            out['year_ma'] = last_year
            out['year_label'] = '年線(MA240)' if interval == '1d' else '年線(52週)'
    except Exception as e:
        out['error'] = str(e)
...
@app.route('/api/predict/kline/<code>')
def predict_kline(code):
    """K 線資料（JSON）：供全站詳情面板繪製蠟燭圖。
    ?tw=1 台股 / ?tw=0 美股；?period=3mo|6mo 日K 期間；?interval=1d|1wk 日K/週K。"""
    is_tw_q = request.args.get('tw')
    code_c  = code.strip().upper().replace('.TW', '').replace('.TWO', '')
    if is_tw_q is None:
        is_tw = code_c.isdigit() or code_c.startswith('00')
    else:
    ...
    def generate():
        try:
            ...
            with client.messages.stream(
                ...
                max_tokens=8192,   # 報告分五大段（繁中），2048 會在中途被截斷；串流模式可放寬，8192 足以寫完整份
                system=_PREDICT_SYSTEM_PROMPT,
                messages=send_msgs,
            ) as stream:
                for text in stream.text_stream:
                    yield f"data: {json.dumps({'text': _strip_emoji(text)})}\n\n"
                # 若仍因長度截斷，明確告知前端，避免使用者誤以為當機（而去請 AI「繼續」卻拿不到即時數據）
                final = stream.get_final_message()
                if final.stop_reason == 'max_tokens':
                    note = '\n\n（內容已達單次長度上限，如需補完請輸入「請接續上一段」）'
...
def _norm_code(code: str) -> str:
    return (code or '').strip().upper().replace('.TW', '').replace('.TWO', '')


@app.route('/api/predict/report/<code>', methods=['GET'])
def predict_report_get(code):
    """讀取某支股票已存的 AI 報告（含對話與已渲染 HTML）。不存在則 found=False。"""
    code = _norm_code(code)
    row = None
    try:
        conn = _sqlite3.connect(_CACHE_DB, timeout=1)
        row = conn.execute('SELECT name, data, msgs, html, updated FROM reports WHERE code=?', (code,)).fetchone()
        conn.close()
    except Exception:
        row = None
    if not row:
...
@app.route('/api/predict/report/save', methods=['POST'])
def predict_report_save():
    """存／覆蓋某支股票的 AI 報告。前端在每次 AI 回覆完成後呼叫。"""
    body = request.get_json(force=True)
    ...
    msgs = json.dumps(body.get('messages') or [], ensure_ascii=False)
    html = body.get('html') or ''
    try:
        conn = _sqlite3.connect(_CACHE_DB, timeout=2)
        conn.execute('INSERT OR REPLACE INTO reports (code, name, data, msgs, html, updated) VALUES (?,?,?,?,?,?)',
                     (code, name, data, msgs, html, time.time()))
        conn.commit(); conn.close()
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    return jsonify({'ok': True})
...
@app.route('/api/predict/reports', methods=['GET'])
def predict_reports_list():
    """列出所有已存報告（依更新時間新到舊），供歷史清單切換。"""
    out = []
    try:
        conn = _sqlite3.connect(_CACHE_DB, timeout=1)
        rows = conn.execute('SELECT code, name, updated FROM reports ORDER BY updated DESC').fetchall()
        conn.close()
        out = [{'code': r[0], 'name': r[1], 'updated': r[2]} for r in rows]
    except Exception:
        pass
    return jsonify(out)


@app.route('/api/predict/report/delete', methods=['POST'])
def predict_report_delete():
    """刪除某支股票的已存報告。"""
...
量價關係與任何可見的指標數值，並用以下框架分析：月線扣抵環境、初升段防禦檢查、量價動能、法人籌碼。
