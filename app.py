...
def _monitor_ai_advice(code, result, holding):
    ...
    try:
        ...
        sig_line = (f"系統訊號：{result.get('actionCn','')}（信心 {result.get('confidence','-')}）"
                    f"— {result.get('reason','')}")
        ask = ('你是使用者的操盤顧問。請用繁體中文給「3-5 行、精簡可執行」的綜合建議，必須包含：'
               '目前股價與（若有持倉）我的成本與損益、現在該買進/續抱/減碼/賣出、以及具體價位或加減碼方向。'
    ...
    for ticker, settings in list(tickers_cfg.items()):
        try:
            ...
            with _monitor_lock:
                ...
                if (action in ('BUY', 'SELL', 'AVOID') and action_changed and cooldown_ok
...
def _etf_recommendation_text(data: dict, holding: dict = None) -> str:
    ...
    if holding:
        ...
        lines.append(f"持倉資訊：買入均價 {buy_p} 元 / {shares} 股 / 未實現損益 {pnl:+.2f}%")
...
def _agent_recommendation_text(data: dict, holding: dict = None) -> str:
    ...
    if data.get('is_etf'):
        return _etf_recommendation_text(data, holding)
        ...
        try:
            ...
            # Intraday scans (every 30 min during market hours)
                ...
                if (now - last_scan_ts).total_seconds() >= 1800:
                    ...
                    urgent_codes = sorted(h['code'] for h in urgent)
                    # 去重複：同一個賣出/減碼訊號不要整個盤中每 30 分鐘重發。
                    # 每天重新基準，只有「出現新的賣出/減碼標的」時才推播盤中警示；
                    # 沒有新訊號時照樣更新掃描資料給前端，但不發 LINE。
                    prev = cfg.get('last_intraday_urgent', []) if cfg.get('last_intraday_date') == today else []
                    new_codes = [c for c in urgent_codes if c not in prev]

                    cfg['last_scan']            = now.strftime('%Y-%m-%d %H:%M')
                    cfg['candidates']           = _run_agent_scan(cfg)
                    cfg['last_intraday_urgent'] = urgent_codes
